...
- `layout-cases-v1.json` is the generator-owned canonical projection of the
  ...
  and independently computed Unicode facts, but no screenshots, runtime
  observations, model predictions, or expected publisher output.
- `runtime-observations-2026.3.1-1885-81193b80fa-v1.json` is a build-scoped
  selection of 52 high-confidence observations joined to the corpus by Case ID
  and payload SHA-256. Forty-nine observations are layout-trace comparisons;
  three document why explicit product-side preparation is required.
...
After a VRChat update, add a new build-named observation file and keep prior
files intact. A new result must not silently rewrite evidence from an older
build. Raw screenshots, logs, extracted game resources, and analysis tools stay
in the separate local research workspace and are not test dependencies.
