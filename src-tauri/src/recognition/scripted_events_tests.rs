...
fn scripted_unit_emits_monotonic_full_snapshots_and_one_completion() {
    let events = ScriptedRecognitionEvents::new(context()).script_unit(
        "unit-live",
        200,
        &[
...
#[test]
fn scripted_abort_emits_an_explicit_terminal_event_without_a_caption() {
    let events = ScriptedRecognitionEvents::new(context()).script_aborted(
        "unit-empty",
        400,
        RecognitionUnitAbortReason::NoSpeech,
    );
