...
experience, a capability planner checks whether the selected model can
honestly deliver it, and an incompatible choice is explained with explicit
alternatives — keep the model and pick a supported mode, or keep the mode and
pick a compatible model. The app never silently switches model or mode, and
never invents a completion the model did not produce.
